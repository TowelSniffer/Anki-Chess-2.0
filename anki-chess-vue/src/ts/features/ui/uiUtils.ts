...
const btn = {
  ...
  get back() {
    return document.querySelector<HTMLButtonElement>("#navBackward");
  },
  get forward() {
    return document.querySelector<HTMLButtonElement>("#navForward");
  },
  get copy() {
    return document.querySelector<HTMLButtonElement>("#copyFen");
  },
  get stockfish() {
    return document.querySelector<HTMLButtonElement>("#stockfishToggle");
  },
  get flip() {
...
import type { CustomShape } from "../../core/types";

// Helper function to convert CustomShape to DrawShape
export function convertCustomShapesToDrawShapes(
  ...
  return customShapes.map((cs) => {
    const ds: DrawShape = {
      orig: cs.orig,
      dest: cs.dest,
      brush: cs.brush || "green", // Provide a default brush if undefined
      // Convert customSvg object to string if it's an object
      customSvg:
        typeof cs.customSvg === "object" && cs.customSvg !== null && 'html' in cs.customSvg
          ? { html: cs.customSvg.html, center: cs.customSvg.center }
          : undefined,
    };
    return ds;
  });

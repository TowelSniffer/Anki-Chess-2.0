...
export function addMoveToPgn(move: Move): PgnPath {
  ...
  if (nextMovePath.length === 1) {
    ...
    if (parentLine && parentLine.variations) {
      const varIndex = parentPath.at(-2);
      if (typeof varIndex === 'number') {
        if (!parentLine.variations[varIndex]) {
          parentLine.variations[varIndex] = [];
        }
        parentLine.variations[varIndex].push(newCustomPgnMove);
      }
    }
  }
...
function _augmentAndGenerateFen(
  ...
  for (let i = 0; i < moves.length; i++) {
    const move = moves[i];
    if (!move) continue;
    console.log("Initial move:", move); // Added console log
    ...
    console.log("Augmented move:", move); // Added console log
